        ...
        <main className="space-y-5">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input list={`case-options-${part}`} value={caseNumber} onChange={(event) => handleCaseInput(event.target.value)} placeholder={casePlaceholder} className="h-12 w-full rounded-2xl border border-slate-200 bg-white pl-10 pr-4 text-sm outline-none focus:border-emerald-400 focus:ring-4 focus:ring-emerald-100 dark:border-slate-700 dark:bg-slate-950 dark:focus:ring-emerald-950" />
                    <datalist id={`case-options-${part}`}>{catalog.map((entry) => <option key={entry.caseId} value={entry.caseId}>{entry.title}</option>)}</datalist>
                  </div>
                </label>
                ...
                {part !== "B" ? (
                  <label className="space-y-2">
                    ...
                    <input list={`item-options-${part}`} inputMode="numeric" value={itemNumber} onChange={(event) => { setItemNumber(event.target.value); setResult(null); }} placeholder="Type or select item" className="h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm outline-none focus:border-emerald-400 focus:ring-4 focus:ring-emerald-100 dark:border-slate-700 dark:bg-slate-950 dark:focus:ring-emerald-950" />
                    <datalist id={`item-options-${part}`}>{Array.from({ length: 10 }, (_, index) => <option key={index + 1} value={String(index + 1)} />)}</datalist>
                  </label>
                ) : (
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-800 dark:bg-slate-950/50"><p className="text-xs font-semibold">PDF order</p><p className="mt-1 text-xs leading-5 text-slate-400">Questions remain in source order from 1 to 171.</p></div>
                )}
              </div>
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/40">
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-2 text-xs text-slate-400"><FileSearchIcon className="size-4" /> Saved answer is reused unless Refresh is pressed.</div>
              <button type="button" disabled={loading} onClick={() => void analyse(false)} className="inline-flex min-w-40 items-center justify-center gap-2 rounded-2xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white shadow-sm hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-200">
                {loading ? <Loader2Icon className="size-4 animate-spin" /> : <SparklesIcon className="size-4" />} Generate answer
              </button>
            </div>
          </section>
        </main>

        <div className="xl:sticky xl:top-6 xl:self-start"><AnswerCard result={result} loading={loading} onRefresh={() => void analyse(true)} /></div>
      </div>
    </div>
  );
}
